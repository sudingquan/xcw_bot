...
def gen_report(gid, uid, nickname, type=REPORT_UNDECLARED, kpi=False):
    ...
    add_text(img, row1, position=(390,625), textsize=35, textfill='black')
    add_text(img, row2, position=(843,625), textsize=35, textfill='black')
    add_text(img, year, position=(365,438), textsize=40, textfill='black')
    ...
    if len(clanname) <= 7:
        add_text(img, clanname, position=(310+(7-len(clanname))/2*40, 505), textsize=40, textfill='black')
    else:
        add_text(img, clanname, position=(310+(10-len(clanname))/2*30, 510), textsize=30, textfill='black')
    add_text(img, nickname, position=(290,360), textsize=35, textfill='white')
    #输出
    buf = BytesIO()
